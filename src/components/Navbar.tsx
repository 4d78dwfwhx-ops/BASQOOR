      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-gray-300 hover:text-gold font-medium transition-colors duration-300 relative group"
              >
                {link.name}
          ...
          <div className="hidden md:flex items-center gap-4">
            <button className="p-2.5 rounded-xl glass hover:bg-gold/10 transition-all duration-300 group">
              <Search className="w-5 h-5 text-gray-300 group-hover:text-gold transition-colors" />
            </button>
            <button className="p-2.5 rounded-xl glass hover:bg-gold/10 transition-all duration-300 group relative">
              <ShoppingCart className="w-5 h-5 text-gray-300 group-hover:text-gold transition-colors" />
              <span className="absolute -top-1 -left-1 w-5 h-5 bg-crimson rounded-full text-xs flex items-center justify-center font-bold">
                0
              </span>
            </button>
            ...
            <div className="flex flex-col gap-3">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-gray-300 hover:text-gold font-medium py-2 px-4 rounded-xl hover:bg-gold/10 transition-all"
                  onClick={() => setIsOpen(false)}
                >
                  {link.name}
                </a>
              ))}
              <button className="btn-primary py-3 px-6 rounded-xl mt-2">
                تسجيل الدخول
              </button>
            </div>
          </motion.div>
